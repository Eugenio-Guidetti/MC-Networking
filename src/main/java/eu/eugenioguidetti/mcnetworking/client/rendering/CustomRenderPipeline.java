...
public class CustomRenderPipeline implements ClientModInitializer
{
    ...
    private static final Set<BlockPos> activeWaypoints = ConcurrentHashMap.newKeySet();
    private static final List<WaypointRenderState> extractedStates = new ArrayList<>();
    ...
    private static void addWaypoint(BlockPos pos)
    {
        activeWaypoints.add(pos);
    }

    private static void removeWaypoint(BlockPos pos)
    {
        activeWaypoints.remove(pos);
    }
    ...
    private void extractWaypoint(LevelExtractionContext context)
    {
        ...
        // Svuotiamo i vecchi dati della GPU
        extractedStates.clear();

        // Copiamo le coordinate attuali e le trasformiamo in stati di rendering (es. cubo verde traslucido)
        for (BlockPos pos : activeWaypoints)
        {
            extractedStates.add(new WaypointRenderState(pos.getX(), pos.getY(), pos.getZ(), 0f, 1f, 0f, 0.5f));
        }
    }

    // :::custom-pipelines:extraction-phase
    // :::custom-pipelines:drawing-phase
    private void renderAndDrawWaypoints(LevelRenderContext context)
    {
        this.renderWaypoints(context);
    ...
    private void renderWaypoints(LevelRenderContext context)
    {
        if (extractedStates.isEmpty())
        {
            return;
        }
        ...
        for (WaypointRenderState waypointState : extractedStates)
        {
            this.renderFilledBox(matrices.last().pose(),
                                 this.buffer,
                                 waypointState.x(),
                                 waypointState.y(),
                                 waypointState.z(),
    ...
    private void renderFilledBox(Matrix4fc positionMatrix,
